...
```bash
cd mmo-cr-catch-recording-service
nvm use
```
...
```bash
docker build --no-cache --tag mmo-cr-catch-recording-service .
```

Run:

```bash
docker run -e PORT=3001 -p 3001:3001 mmo-cr-catch-recording-service
```
